...
- Finding victim's windows host name  using nbns(netbios name server),SMB and SMB2. Therefore  this filter in Wireshark is displaying packets that are related to either NetBIOS Name Service (NBNS), Server Message Block (SMB), or SMB2 protocol activity. This filter is used for analyzing network traffic in Windows environments as shown
